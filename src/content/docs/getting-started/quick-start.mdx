...
When you run **any** loko command for the first time, loko automatically checks if required tools (Docker, git, kind, kubectl, helm, cfssl, helmfile) are installed. If any are missing, you'll see clear installation guidance. Use `loko check prerequisites` to manually check at any time.
:::
