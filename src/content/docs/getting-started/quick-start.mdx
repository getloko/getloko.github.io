...
**Missing something?** → [Installation Guide](/getting-started/installation/)

:::note[Checking prerequisites]
LoKO does not check prerequisites automatically before other commands. Run `loko check prerequisites` explicitly any time to verify Docker, git, kind, kubectl, helm, cfssl, and helmfile are installed.
:::
...
**Expected output** (`--short` table form; the default renders one rounded panel per component):
```
STATUS
COMPONENT   STATUS    SUMMARY
cluster     healthy   dev-mac running
registry    healthy   enabled
```

Check your enabled workloads:
...
pgadmin              Running      1/1                bjw-s/app-template
```
...
This prints formatted sections per workload (DB credentials, connection info). For raw JSON instead, pass `--all`:
...
**See all available workloads**: [Workload Catalog](/_catalog/workloads/)
...
| Browse the workload catalog | `loko catalog list` |
| Check your enabled workloads | `loko workloads health` |
...
| Destroy cluster | `loko env destroy --force` |
...
See [Troubleshooting Guide](/reference/troubleshooting/) or [open an issue](https://github.com/getloko/loko-releases/issues).
...
**Ready to dive deeper?** [Understand the architecture →](/architecture/)
