...
Complete reference for the `loko.yaml` configuration file schema. Fields are
sourced from the config structs in `internal/config` and validated with
...
| `dns-port` | int\|null | No | auto | dnsmasq host port, auto-selected from non-privileged ports (1024–32767), preferring 5453. Rarely needs to be set manually. |
...
| `nodes.control-planes` | int | No | `1` | Number of control-plane nodes. Values above `1` require `isolate-internal-components: false` (see scheduling below). |
...
| `scanning.enabled` | bool | No | `false` | Enable Zot CVE scanning (Trivy-backed) via `search.cve`. |
| `scanning.interval` | string | No | `"24h"` | Trivy database update interval (for example `12h`, `24h`). |
...
| `dnsmasq.version` | string\|null | No | `null` | dnsmasq image version override. |
