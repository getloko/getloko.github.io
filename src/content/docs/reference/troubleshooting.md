...
This guide covers common issues and their solutions for LoKO. Start with the diagnostic workflow, then find your issue in the sections below.
...
# Per-workload readiness (helm release + pod status)
loko workloads health

# DNS service + OS resolver
loko check dns
```

**Health States** (`loko status`):

- **healthy** - Component running normally
- **degraded** - Running but has issues
- **stopped** - Component not running
- **not_configured** - Component not set up

`loko status` reports the **cluster** and, when enabled, the **registry**.
DNS is checked with `loko check dns`; deployed workloads with
`loko workloads health`.
...
docker logs loko-<env>-dns          # DNS container (no `loko logs` subcommand for it)
...
# Check DNS container logs (docker directly — no `loko logs` subcommand for DNS)
docker logs loko-<env>-dns
...
**Root Cause:** Required tools not installed on system. LoKO does not check prerequisites automatically on other commands — run `loko check prerequisites` explicitly.
...
# Set cluster.kubernetes.api-port: 6444 in loko.yaml, then re-run
loko init
```
...
# Retry deployment (deploy is idempotent; disable first to force a clean reinstall)
loko workloads disable <name> && loko workloads enable <name>
...
# Redeploy workload (deploy is idempotent)
loko workloads deploy <name>
```
...
### Issue 10: Confused by `loko create` vs `loko env create`

**Symptom:** Unsure which form to use, or a script mixes both

**Explanation:** These are not different commands. `loko create`, `loko start`, `loko stop`, `loko destroy`, `loko clean`, `loko recreate`, `loko restart`, and `loko activate` are permanent top-level aliases for the identical `loko env <verb>` subcommands — both forms call the same underlying function. Neither is deprecated; use whichever reads better in context.
...
**Native support.** LoKO ships native binaries for Windows (amd64/arm64) as well as macOS and Linux. WSL2 is not required.
...
```bash
# Use the --debug flag for full command tracing detail
