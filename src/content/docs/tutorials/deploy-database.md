...
**Output**:
```
Added enabled workload 'postgres' to loko.yaml

Deploy with: loko workloads deploy postgres
```
...
**Output**:
```
Deployed workload(s): postgres
```

That's the entire output — credential generation, namespace creation, PVC, and DNS/routing all happen silently as part of the deploy. Check what actually landed with `loko workloads health` and `loko secrets show`.
...
**Output** (formatted sections; raw JSON via `--all`):
```
postgres:
  username: postgres
  password: 8x9YzA2bC3dE4fG
  database: postgres
```
...
const client = new Client({
  ...
  port: 5432,
  user: 'postgres',
  password: '8x9YzA2bC3dE4fG',  // From secrets.json
  database: 'myapp'
})
