...
```
[1/4] Catalog path resolved...
[2/4] Catalog loaded...
[3/4] Forgejo client created...
[4/4] GitOps provider bootstrapped...
GitOps initialized with provider "fluxcd"
```
...
```
Provider: fluxcd (Ready) healthy=true
Workloads:
  flux-system/clusters-dev: Applied healthy=true last_sync=1m ago
```
...
Provider: fluxcd (Ready) healthy=true
Workloads:
  flux-system/clusters-dev: Applied healthy=true last_sync=5s ago
```
...
There is no interactive confirmation prompt — `loko gitops destroy` runs immediately. Use `--dry-run` first if you want to see what would happen without making changes.

The full teardown completes in about 30 seconds.
