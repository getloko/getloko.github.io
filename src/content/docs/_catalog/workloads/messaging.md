...
```bash
loko workloads add rabbitmq --now
...
```bash
loko workloads add redpanda --now
...
```bash
loko workloads add elasticmq --now
...
Lightweight AWS SNS-compatible messaging service for local event-driven development.

**goaws Features:**

- AWS SNS API compatible
- Simple local topic/publish/subscribe testing
- No AWS account required

**goaws Deployment:**

```bash
loko workloads add goaws --now
loko workloads connect goaws
```

**goaws Endpoints:**

- **SNS API**: `https://sns.${LOKO_DOMAIN}`

**goaws AWS CLI Example:**

```bash
aws sns list-topics \
  --endpoint-url https://sns.${LOKO_DOMAIN} \
  --region us-east-1
```

**goaws Chart:** bjw-s/app-template (admiralpiett/goaws image)
...
**Deployment:**
```bash
loko workloads add nats-ui --now
```
