...
make build          # produces ./loko (unobfuscated, with symbols)
```
...
Format with `gofmt` (the standard Go formatter) before committing:
...
- Deployed with `loko workloads add minio --now`
...
- [ ] Workload deploys successfully with `loko workloads add <name> --now`
...
Workload definitions are schema-validated by the LoKO CLI on load. Common validation errors:
