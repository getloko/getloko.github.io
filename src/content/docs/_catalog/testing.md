...
```bash
# Point LoKO to your local catalog, add the workload, deploy it
LOKO_CONFIG_DIR=/path/to/test-config loko workloads add myapp
...
```bash
# Check readiness of all enabled workloads
...
```bash
# Both myapp and its linked UI should show ready
loko workloads health
```
...
loko config sync --diff   # preview generated helmfile changes
```
