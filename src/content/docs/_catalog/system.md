...
- **Components**: Internal system components (CoreDNS + dns-operator, HAProxy, Traefik, metrics-server, Zot)
...
```mermaid
graph LR
    Remote[🌐 Remote Catalog<br/>HTTPS URL] --> Global[💾 Global Cache<br/>~/.config/loko/catalog.yaml]
...
**Location**: `~/.config/loko/catalog.yaml`
...
├── components.yaml           # System components (dns-operator, HAProxy, etc.)
├── workloads/
