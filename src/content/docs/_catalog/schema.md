...
Workloads are defined using YAML and validated against Go struct definitions in the LoKO CLI. This reference covers all available fields and their usage.
...
| `${LOKO_APPS_DOMAIN}` | `dev.me` | Apps domain |
| `${LOKO_IP}` | `192.168.0.10` | Local IP address |
...
All workload definitions are validated using hand-rolled Go struct validation in the LoKO CLI (`internal/config`, `internal/catalog`), not a third-party schema library.
