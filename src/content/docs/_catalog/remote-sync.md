...
graph TD
    ...
    style ValidateSchema fill:#f3e5f5
...
- Cache location: `~/.config/loko/.cache/remote-catalogs/<sha256-hash>/`
- Automatic expiry after TTL
- Cache bypass available

**Cache Structure**:
```
~/.config/loko/.cache/remote-catalogs/
...
rm -rf ~/.config/loko/.cache/remote-catalogs/
```
