...
- Creates TLS certificates with mkcert
- Starts DNS service
...
└── workload-secrets.json   # Generated credentials
```
...
loko config generate --name dev-project-a
loko config generate --name dev-project-b
...
```bash
# Backup secrets
cp .loko/dev-me/workload-secrets.json ~/backups/
