...
Roughly, it runs these phases in order:

```bash
loko init                # config artifacts + network (no cluster)
# ...then creates the Kind cluster,
# ...deploys the internal components (Traefik, Zot, metrics-server),
loko workloads deploy    # ...and every enabled workload
```

`loko init` is the only one of these you'd normally run on its own.
...
```bash
loko status              # uses ./loko.yaml
loko status -c path/to/loko.yaml
```

`loko status` reports the **cluster** and (when enabled) the **registry**. It
prints one line per component, plus a `-> guidance` line when there is a
suggested next step:
...
Registry: not_configured
  -> Enable registry in loko.yaml to push/pull images locally
```
...
|-------|---------|-------------------|
| `healthy` | Running and functional | None |
| `degraded` | Running but a check failed | Inspect nodes / pod logs (guidance line points where) |
| `stopped` | Exists but not running | `loko start` (or `loko create` if never created) |
| `not_configured` | Not enabled in `loko.yaml` | Enable it in config |

For per-workload readiness use `loko workloads health`; for DNS use
`loko check dns`.
...
- Deploy every enabled workload: `loko workloads deploy`
- Deploy a specific workload: `loko workloads deploy <name>`
...
Environment Status
Cluster: stopped
  -> Run 'loko create' to create the cluster
Registry: not_configured
  -> Enable registry in loko.yaml to push/pull images locally
```

Next step: `loko create`.
...
Next step: `loko start` to bring the containers back up (this also restores the
registry).

#### Scenario 3: Post-Reboot

The cluster containers are stopped after a reboot. Run `loko start`, then
`loko check dns` to confirm the OS resolver is still wired up.
...
`loko status` prints plain text and exits non-zero when it cannot read the
config. For scripted health checks, drive `kubectl` directly against the
cluster or grep the status output:

```bash
# Fail a CI step if the cluster is not healthy
loko status | grep -q '^Cluster: healthy' || { echo "cluster not healthy"; exit 1; }

# Per-workload readiness
...
```bash
loko status              # cluster + registry health (see "Health Status Command" above)
loko check dns           # DNS service + OS resolver
loko workloads health    # per-workload readiness
loko inventory list      # which environment is active
```
...
```bash
# Backup config + persisted state (inventory, secrets, generated manifests)
cp loko.yaml ~/backups/loko.yaml.backup
cp -r ~/.loko ~/backups/loko-state-$(date +%Y%m%d)
...
# Redeploy specific workload (deploy is idempotent)
loko workloads deploy postgres
