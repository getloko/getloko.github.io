...
network:
  ...
  subdomain:                    # Subdomain for user apps
    enabled: boolean
    value: string               # Subdomain name (e.g., apps)
...
network:
  ...
  subdomain:
    enabled: true             # Use subdomain for user apps
    value: apps               # Subdomain name
```

**Resulting DNS:**
- System: `postgres.mydev.local`
- User: `myapp.apps.mydev.local` (if subdomain enabled)
- User: `myapp.mydev.local` (if subdomain disabled)
