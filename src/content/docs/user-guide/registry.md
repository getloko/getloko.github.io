...
:::note
After a registry restart or config reapply, CVE results can be temporarily incomplete.
Zot first syncs Trivy databases, then background scanning catches up. During this window, the UI may briefly show statuses like `Failed to scan` or empty vulnerability data. Refresh after a few minutes.
:::
...
```bash
loko registry repo myapp
loko registry repo docker.io/library/nginx
```
...
Not currently configurable via `loko.yaml` — the registry always allows anonymous push/pull/list, matching the "No Authentication Required" behavior above. There is no `registry.auth` (or similar) field in the current schema.
...
```bash
# Check registry pod
kubectl get pods -n loko-system -l app=zot

# Check registry service
kubectl get svc -n loko-system zot
...
```bash
yq '.registry.mirroring' loko.yaml

loko logs k8s deploy/zot -n loko-system | grep -i mirror
...
loko registry repos | grep docker.io
