...
```
loko secrets (workload-secrets.json)
    ↓
...
**Solution:**
- Generator creates and persists a random 32-char admin token in `workload-secrets.json`
...
def _get_or_generate_admin_token(self, workload_name: str, length: int = 32) -> str:
    """Get existing admin token from secrets file or generate and persist a new one."""
...
_get_or_generate_admin_token('garage')
    ↓
workload-secrets.json['garage']['admin_token'] = "lVGnJBx4..."
    ↓
...
- **S3 credentials**: Created via `garage key create` during post-deploy configuration (stored in workload-secrets.json)
