...
LoKO ships as a single statically-linked binary (Go, `CGO_ENABLED=0`) with no
runtime dependencies of its own — it drives `docker`, `kind`, `kubectl`,
`cfssl`, and `helmfile`, which you install separately.
